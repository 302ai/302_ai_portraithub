...
export default function MuseumLostForm() {
  ...
  const [model, setModel] = useState(
    museumLostConfig?.defaultModel || "gemini-2.5-flash-image-preview"
  );

  // 初始化表单数据
  useEffect(() => {
    const initialData: Record<string, string> = {};
    museumLostConfig?.parameters?.forEach((param) => {
  ...
  const handleGenerate = async () => {
    ...
    const finalPrompt =
      typeof museumLostConfig?.prompt === "function"
        ? museumLostConfig.prompt(processedFormData as any, model)
        : "";
      ...
      <div className="flex flex-col gap-6 md:flex-row">
        ...
        <div className="mt-4 flex w-full flex-1 flex-col justify-between space-y-12 md:ml-4 md:mt-0 md:w-1/2">
          <div className="space-y-6">
            {/* 动态生成表单参数 */}
            {museumLostConfig?.parameters?.map((param) => {
                  ...
                  <div key={param.key} className="flex items-center space-x-3">
                    ...
                    >
                      <SelectTrigger id={param.key} className="flex-1">
                        <SelectValue
                          placeholder={t(
                            `museum-lost.placeholder.${param.key}`
                          )}
                        />
                      </SelectTrigger>
                      <SelectContent>
