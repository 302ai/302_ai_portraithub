...
export default function LightPaintingPortraitForm() {
  ...
  // 获取light-painting-portrait的配置
  const lightPaintingPortraitConfig = pageExamples.find(
    (item) => item.id === "light-painting-portrait"
  );

  const [formData, setFormData] = useState<Record<string, string>>({});
  const [model, setModel] = useState(
    lightPaintingPortraitConfig?.defaultModel || "doubao-seedream-4-0-250828"
  );

  // 初始化表单数据
  useEffect(() => {
    const initialData: Record<string, string> = {};
    lightPaintingPortraitConfig?.parameters?.forEach((param) => {
      initialData[param.key] = (param as any).defaultValue || "";
    });
    setFormData(initialData);
  }, [lightPaintingPortraitConfig]);
  ...
  const handleGenerate = async () => {
    ...
    const finalPrompt =
      typeof lightPaintingPortraitConfig?.prompt === "function"
        ? lightPaintingPortraitConfig.prompt({} as any, model)
        : "";
        ...
        <div className="mt-4 flex w-full flex-1 flex-col justify-between space-y-12 md:ml-4 md:mt-0 md:w-1/2">
          ...
          <div className="flex items-center space-x-3">
            <div className="flex-1 text-sm">
              {t("light-painting-portrait.description")}
            </div>
          </div>
