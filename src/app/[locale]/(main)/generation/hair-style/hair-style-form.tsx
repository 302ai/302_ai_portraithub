...
export default function HairStyleForm() {
  ...
  // 获取hair-style的配置
  const hairStyleConfig = pageExamples.find((item) => item.id === "hair-style");

  const [formData, setFormData] = useState<Record<string, string>>({});
  const [model, setModel] = useState(
    hairStyleConfig?.defaultModel || "gemini-2.5-flash-image-preview"
  );

  // 只显示Gemini模型
  const geminiModels = models.filter((model) => model.includes("gemini"));

  // 初始化表单数据
  useEffect(() => {
    const initialData: Record<string, string> = {};
    hairStyleConfig?.parameters?.forEach((param) => {
  ...
  const handleGenerate = async () => {
    ...
    const finalPrompt =
      typeof hairStyleConfig?.prompt === "function"
        ? hairStyleConfig.prompt({} as any, model)
        : "";
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="flex flex-col gap-6 md:flex-row">
        ...
        <div className="mt-4 flex w-full flex-1 flex-col justify-between space-y-12 md:ml-4 md:mt-0 md:w-1/2">
          ...
          {/* 提示文字 */}
          <div className="flex items-center space-x-3">
            <div className="flex-1 text-sm">{t("hair-style.description")}</div>
          </div>
