...
export default function ComicPortraitForm() {
  ...
  const [model, setModel] = useState(
    comicPortraitConfig?.defaultModel || "doubao-seedream-4-0-250828"
  );

  // 初始化表单数据
  useEffect(() => {
    const initialData: Record<string, string> = {};
    comicPortraitConfig?.parameters?.forEach((param) => {
  ...
  const handleGenerate = async () => {
    ...
    const finalPrompt =
      typeof comicPortraitConfig?.prompt === "function"
        ? comicPortraitConfig.prompt({} as any, model)
        : "";
